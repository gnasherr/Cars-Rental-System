...
class Predictor:
    ...
    def __init__(self) -> None:
        self.data = pd.read_csv('D:/mini/Cars/app/cars.csv',thousands=',')
    ...
    def preprocess(self):
        ...
        self.data['company'] = self.data['full_name'].str.split(' ', expand=True)[0]
        ...
        for i,val in enumerate(self.data['max_power']):
            ...
            if val == 'null ':
                self.data[:] = self.data.drop(i)
        self.data = pd.get_dummies(data=self.data, columns=['seller_type','fuel_type','transmission_type','company'], drop_first=True)
        ...
        self.X = self.X.drop(['selling_price','full_name','owner_type'],axis=1)
        self.X['engine'] = pd.to_numeric(self.X['engine'])
        self.X['max_power'] = pd.to_numeric(self.X['max_power'])
        self.X = self.X.dropna()
        self.split()
    def split(self):
        self.xtrain, self.xtest, self.ytrain, self.ytest = train_test_split(self.X,self.Y,test_size = 0.2, random_state = 1)
    ...
    def cross_val(self,xtrain, ytrain):
        accuracies = cross_val_score(estimator = self.classifier, X = xtrain, y = ytrain, cv = 5)
